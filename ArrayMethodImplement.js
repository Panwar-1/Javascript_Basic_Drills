...
reduce(items, (startingValue, value) => {
...
find([1, 3, 5, 7, 9, 10, 4], (array) => {
    let number = array.find((value => value % 2 === 0));
    if (!number) { return undefined }
    else { return number }
});

find([1, 3, 5, 7, 9, 11, 13], (array) => {
    let number = array.find(value => value % 2 == 0);
    if (!number) {
        return "undefined";
    } else {
...
function filter(elements, cb) {
    ...
    for (let index = 0; index < elements.length; index++) {
        if (cb(elements[index]) == true) {
            filterData.push(elements[index]);
        }
    }
    // console.log(filterData); [11, 13, 15]
    return filterData;
}

filter([0, 2, 4, 8, 11, 13, 15], (element) => {
    if (element % 2 !== 0) {
        return true;
    }
});
